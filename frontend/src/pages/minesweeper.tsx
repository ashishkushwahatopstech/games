...
function MinesweeperPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${backendUrl}/api/leaderboard/minesweeper`)
  ...
  const submitScore = async (score: number) => {
    if (!token) return;
    try {
      await fetch(`${backendUrl}/api/leaderboard/minesweeper`, {
        method: "POST",
        headers: {
  ...
  return (
    <PageLayout pageTitle="Minesweeper">
      <MinesweeperBlitz 
