...
function SnakeLadderPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${backendUrl}/api/leaderboard/snakeladder`)
  ...
  const submitScore = async (score: number) => {
    if (!token) return;
    try {
      await fetch(`${backendUrl}/api/leaderboard/snakeladder`, {
        method: "POST",
        headers: {
  ...
  return (
    <PageLayout pageTitle="Snakes & Ladders">
      <SnakeLadder 
      ...
      />
      <GameGuide gameId="retro-snake" /> {/* Uses retro-snake how to play guidelines */}
    </PageLayout>
  )
}
